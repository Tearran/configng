...
network_options+=(
	...
	["module_simple_network,doc_link"]="tools/modules/network/module_network_simple_refactored.sh"
...
function module_simple_network() {
	# Deprecation stub: delegate to refactored implementation
	...
	local title="network"
	local condition
	condition=$(which "$title" 2>/dev/null)
	...
	case "$1" in
					...
					if [[ "${wiredmode}" == "dhcp" ]]; then
						...
						for f in /sys/class/net/*; do
							...
							if [[ $intf =~ ^dummy0|^lo|^docker|^virbr ]]; then
								...
								local tmp
								tmp=$(ip -4 addr show dev "$intf" | grep "$adapter" | grep -v "$intf:avahi" | awk '/inet/ {print $2}' | uniq | head -1)
								[[ -n $tmp ]] && ips+=("$tmp")
							fi
						done
						address=${ips[0]}
		...
		"$SELECT_CMD")
			...
			declare -A CHANNELS=(
				...
				['5680']='136' ['5700']='140' ['5720']='144' ['5745']='149' ['5765']='153'
				['5785']='157' ['5805']='161' ['5825']='165'
			)
			default_IFS=$IFS
			IFS=$'\n'
			local iw_command=(
				$(lc_all=C sudo iw dev "${adapter}" scan | grep -o 'BSS ..\:..\:..:..\:..\:..\|SSID: .*\|signal\: .* \|freq\: .*')
			)
			IFS=$default_IFS
			COUNT=1
			for line in "${iw_command[@]}"; do
				...
				IFS=" 	"
				if [[ $line =~ BSS ]]; then
					bss_array=( $line ); bssid=${bss_array[1]}
				fi
				if [[ $line =~ "freq:" ]]; then
					freq_array=( $line ); freq=$(echo ${freq_array[1]} | cut -d"." -f1)
				fi
				if [[ $line =~ "signal:" ]]; then
					signal_array=( $line ); rssi=$(echo ${signal_array[1]} | cut -d"." -f1)
				fi
				if [[ $line =~ "SSID" ]]; then
					ssid_array=( $line ); unset ssid_array[0]; ssid=${ssid_array[@]}
				fi
				if [ $COUNT -eq 4 ]; then
					...
					pair+=(${bssid}="${ssid}")
					COUNT=0
					unset bssid ssid freq rssi channel ssid_array signal_array freq_array bss_array
				fi
				((COUNT++))
				IFS=$default_IFS
			done
			SELECTED_BSSID=$($DIALOG --notags --title "Select SSID" --menu "\nSSID                     Signal   Frequency Channel" $((${#list[@]}/2+10)) 57 $((${#list[@]}/2)) "${list[@]}" 3>&1 1>&2 2>&3)
			if [[ $? -eq 0 ]]; then
				for elt in "${pair[@]}"; do
		...
		"$STORE_CMD")
			local list=()
			for f in /sys/class/net/*; do
				local interface; interface=$(basename "$f")
				if [[ $interface =~ ^dummy0|^lo|^docker|^virbr|^br ]]; then
					continue
				else
					[[ $interface == w* && $interface != wa* ]] && devicetype="wifi" || devicetype="wired"
					local query; query=$(ip -4 -br addr show dev "$interface" | awk '{print $3}')
					list+=("${interface}" "$(printf "%-16s%18s%9s" "$interface" "${query:-unasigned}" "$devicetype")")
				fi
			done
			adapter=$($DIALOG --notags --title "Select interface" --menu "\nAdaptor                 IP address     Type" $((${#list[@]}/2+10)) 50 $((${#list[@]}/2+1)) "${list[@]}" 3>&1 1>&2 2>&3)
			if [[ $? -eq 0 ]]; then
		...
		"$REMOVE_CMD")
			...
			} $1' /etc/netplan/"${yamlfile}".yaml > /etc/netplan/"${yamlfile}".yaml.tmp
